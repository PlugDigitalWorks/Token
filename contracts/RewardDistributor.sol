...
contract RewardDistributor is IRewardDistributor, ReentrancyGuard {
    ...
    mapping (address => uint256) rewardRecipientIndexes;
    ...
    modifier onlyToken() {
        require(msg.sender == rewardToken); _;
    }
    ...
    // EVENTS
    event SetRewardTokenAddress(address rewardToken);
    event TransferedTokenOwnership(address newOwner);
    event UpgradeDistributor(address newDistributor);
    event AddRewardHolderShare(address rewardRecipient, uint256 amount);
    event RemoveRewardholder(address rewardRecipient);
    event GiftReward(address rewardRecipient, address giftRecipient, uint256 amount);
    event ClaimReward(address rewardRecipient);
    event ClaimRewardInDesiredToken(address rewardRecipient, address desiredToken);
    event ClaimRewardToDesiredWallet(address rewardRecipient, address desiredWallet);
    event ClaimRewardInDesiredTokenToDesiredWallet(address rewardRecipient, address desiredWallet, address desiredToken);
    ...
    function setRewardTokenAddress(address _rewardToken) external onlyOwner {
        ...
        if (rewardToken != address(0)) {
            ...
            if (balance > 0) {
                IERC20(rewardToken).transfer(tokenOwner, balance);
            }
        }
    ...
    function upgradeDistributor(address newDistributor) external onlyOwner {
        ...
        uint256 balance = IERC20(rewardToken).balanceOf(address(this));
        if (balance > 0) IERC20(rewardToken).transfer(newDistributor, balance);
    ...
    function addRewardHolderShare(address rewardRecipient, uint256 amount) external override onlyToken {
        ...
        rewardRecipientIndexes[rewardRecipient] = rewardRecipients.length;
        rewardRecipients.push(rewardRecipient);
        emit AddRewardHolderShare(rewardRecipient, amount);
    }
    
    function removeRewardHolderShare(address rewardRecipient) external onlyOwner { 
        totalReferralShares = totalReferralShares.sub(referralShares[rewardRecipient].amount);
        referralShares[rewardRecipient].amount = 0;

        rewardRecipients[rewardRecipientIndexes[rewardRecipient]] = rewardRecipients[rewardRecipients.length-1];
        rewardRecipientIndexes[rewardRecipients[rewardRecipients.length-1]] = rewardRecipientIndexes[rewardRecipient]; 
        rewardRecipients.pop();
        delete rewardRecipientIndexes[rewardRecipient];
        emit RemoveRewardholder(rewardRecipient);
    }
    ...
    function emergencyWithdrawTokens(address token) external onlyOwner {
        ...
        if (balance > 0) {
            IERC20(token).transfer(tokenOwner, balance);
        }
    }
    ...
    function _giftReward(address rewardRecipient, address giftRecipient, uint256 amount) private {
        ...
        if (referralShares[rewardRecipient].amount <= IERC20(rewardToken).balanceOf(address(this))) {
            IERC20(rewardToken).approve(rewardRecipient, amount);
            try IERC20(rewardToken).transfer(giftRecipient, amount) {} catch {}
        }
